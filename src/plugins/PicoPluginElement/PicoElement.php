 ...
 * @version 1.0
 */
class PicoElement extends AbstractPicoPlugin
{
    /**
     ...
     *
     * @param string &$outputawContent raw file contents
     *
     * @return void
     */
    public function on404ContentLoaded(&$outputawContent)
    {
        // your code
    }
     ...
     * @return void
     */
    public function onContentLoaded(&$outputawContent)
    {
        // your code
    }
    ...
    {
        ////////////////////////////////////////////////////////////////////////////////////////////
        //$markdown = mark_convert($markdown);
    }
     ...
     * @return void
     */
    public function onSinglePageContent($id, &$outputawContent)
    {
        // your code
    }
    ...
    public function onPageRendered(&$output)
    {
        ...
        for($i = 0; $i < sizeof($lines); $i++){
            $line = trim($lines[$i]);
            if(substr($line, 0, 16) == "<!--pico-element" and substr($line, -3) == "-->"){
                $mark_name = trim(substr($line, 4, -3));
                if($mark_name == "pico-element-end"){
                    $lines[$i] = "</div>";
                }else{
                    $class_name = $mark_name;
                    $funct_name = str_replace(array("-"," "), "_", $mark_name);
                    $lines[$i] = "<div onchange='" . $funct_name . "()' class='" . $class_name . "'>";
                }
            }
        }
        $output = implode("\r\n", $lines);
        //css
        $link = "<link rel='stylesheet' href='" . $BaseUrl . "plugins/PicoElement/pico-element.css' type='text/css' />";
        $output = str_replace('</head>', $link . '</head>', $output);
        //javascript
        $script = "";
        $script = $script . "<script src='http://ajax.aspnetcdn.com/ajax/jQuery/jquery-1.7.2.min.js' type='text/javascript'></script>\n";
        $script = $script . "<script src='" . $BaseUrl . "plugins/PicoElement/pico-element.js' type='text/javascript'></script>\n";
        $output = str_replace('</body>', $script . '</body>', $output);
        //$output = $output . getBaseUrl();
    }
